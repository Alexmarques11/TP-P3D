...
out vec3 vLightPosEyeSpace; // Posição da luz no espaço da câmera (varying)
...
void main() {
    ...
    // Calcula a posição da luz no espaço da câmera (eye space)
    vec4 lightPosEyeSpace = View * vec4(LightPos, 1.0);
    vLightPosEyeSpace = lightPosEyeSpace.xyz;
